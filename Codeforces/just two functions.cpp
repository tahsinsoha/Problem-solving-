...
public:
    ...
    inline mrix operator + (mrix const &obj)
    {
    ...
    inline void operator = (const mrix & obj )
    {
        I = obj.I;
    }
    inline mrix operator * (mrix const &obj)
    {
        ...
        mrix temp(I.size(), obj.I[0].size(), 0);
        for (int i = 0; i < I.size(); i++)
        {
    ...
    inline mrix operator * (T const &obj)
    {
        mrix temp(I.size(), I[0].size(), 0);
        for (int i = 0; i < I.size(); i++)
        {
            for (int j = 0; j < I[0].size(); j++)
            {
